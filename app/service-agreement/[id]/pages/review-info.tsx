          ...
          <div className="flex flex-col space-y-4 bg-neutral-50 p-4 rounded-md">
            <div className="flex flex-col space-y-1">
              <Label className="text-neutral-500 text-sm">
                {state.companyType === "Other" ? "Company name" : "Strata plan number (CTS/SP/OC)"}
              </Label>
              <span className="text-base break-words">{state.companyName || "N/A"}</span>
            </div>

            <div className="flex flex-col space-y-1">
              <Label className="text-neutral-500 text-sm">ABN</Label>
              <span className="text-base break-words">{state.abn || "N/A"}</span>
            </div>

            <div className="flex flex-col space-y-1">
              <Label className="text-neutral-500">Business address</Label>
              {state.businessStreetAddress && state.businessCity && state.businessPostcode && state.businessState ? (
                <span className="text-base break-words">
                  {state.businessStreetAddress}, {state.businessCity} {state.businessState} {state.businessPostcode}, Australia
                </span>
              ) : (
          ...
          <div className="flex flex-col space-y-4 bg-neutral-50 p-4 rounded-md">
            <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 w-full">
              <div className="flex-1 flex flex-col space-y-1">
                ...
                <span className="text-base break-words">{state.accountFirstName || "N/A"}</span>
              </div>
              <div className="flex-1 flex flex-col space-y-1">
                <Label className="text-neutral-500 text-sm">Last name</Label>
                <span className="text-base break-words">{state.accountLastName || "N/A"}</span>
              </div>
            </div>

            <div className="flex flex-col space-y-1">
              <Label className="text-neutral-500 text-sm">Email address</Label>
              <span className="text-base break-words">{state.accountEmail || "N/A"}</span>
            </div>

            <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 w-full">
              <div className="flex flex-col space-y-1 w-1/2">
                <Label className="text-neutral-500 text-sm">Mobile phone</Label>
                <span className="text-base break-words">{state.accountMobile || "N/A"}</span>
              </div>
              <div className="flex flex-col space-y-1 w-1/2">
                <Label className="text-neutral-500 text-sm">Office phone</Label>
                <span className="text-base break-words">{state.accountPhone || "N/A"}</span>
              </div>
            </div>

            <div className="flex flex-col space-y-1">
              <Label className="text-neutral-500">Postal address</Label>
              {state.postalStreetAddress && state.postalCity && state.postalState && state.postalPostcode ? (
                <span className="text-base break-words">
                  {state.postalStreetAddress}, {state.postalCity} {state.postalState} {state.postalPostcode}, Australia
                </span>
              ) : (
                <span className="text-base break-words">N/A</span>
              )}
