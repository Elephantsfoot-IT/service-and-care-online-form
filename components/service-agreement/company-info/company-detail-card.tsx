...
import { CompanyInfo, ServiceAgreement } from "@/lib/interface";
...
const CompanyDetailsCard = React.forwardRef<CompanyDetailsCardHandle, Props>(
  function CompanyDetailsCard(_props, ref) {
    ...
    const serviceAgreement = state.serviceAgreement as
      | (ServiceAgreement & { company_details?: CompanyInfo })
      | undefined;

    // Use company_details instead of simpro_customer
    const companyDetails = serviceAgreement?.company_details ?? null;
    const hasCompanyDetails = !!companyDetails;

    const schema = useMemo(() => makeCustomerSchema(), [hasCompanyDetails]);

    // Empty defaults when there is no linked company_details
    ...
    // Map from CompanyInfo -> form values
    const valuesFromCompany = (c: CompanyInfo): CustomerDetailsFormType => ({
      abn: c?.abn ?? "",
    ...
    // Initial defaults based on presence of company_details
    const initialDefaults = useMemo(
      () =>
        hasCompanyDetails ? valuesFromCompany(companyDetails!) : emptyValues,
    ...
    // Rehydrate when company_details appears/disappears
    useEffect(() => {
      if (hasCompanyDetails) {
        const vals = valuesFromCompany(companyDetails!);
        form.reset(vals);
